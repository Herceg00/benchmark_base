...
#ifdef __USE_KUNPENG__
template<typename AT>
float Kernel_read(AT **chunk, size_t size)
{
    float return_val = 0;
    #pragma omp parallel
    {
...
#ifdef __USE_KUNPENG__
template<typename AT>
float Kernel_read_and_write(AT **chunk_read, AT **chunk_write, size_t size)
{
    float return_val = 0;
    #pragma omp parallel
    {
...
template<typename AT>
float Kernel(int core_type, AT **chunk_read, AT **chunk_write, size_t size)
{
    if(core_type == 0)
        return Kernel_read(chunk_read, size);
    if(core_type == 1) 
        return Kernel_read_and_write(chunk_read, chunk_write, size);
}
