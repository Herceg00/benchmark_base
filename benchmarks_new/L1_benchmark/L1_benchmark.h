...
#ifdef __USE_KUNPENG__
template<typename AT>
float Kernel_read_and_write(AT *a, AT **chunk, size_t size)
{
    float return_val = 0;
    #pragma omp parallel
    {
        ...
        for(int i = 0; i < RADIUS; i++)
        {
            ...
            data1 = vld1q_f32(chunk_start + offset + 4 * 0);
            local_sum1 = vaddq_f32(local_sum1, data1);
            vst1q_f32(chunk_start + offset + 4 * 0, local_sum1);

            data2 = vld1q_f32(chunk_start + offset + 4);
            local_sum2 = vaddq_f32(local_sum2, data2);
            vst1q_f32(chunk_start + offset + 4 * 1, local_sum1);

            data3 = vld1q_f32(chunk_start + offset + 8);
            local_sum3 = vaddq_f32(local_sum3, data3);
            vst1q_f32(chunk_start + offset + 4 * 2, local_sum1);

            data4 = vld1q_f32(chunk_start + offset + 12);
            local_sum4 = vaddq_f32(local_sum4, data4);
            vst1q_f32(chunk_start + offset + 4 * 3, local_sum1);


            data1 = vld1q_f32(chunk_start + offset + 16);
            local_sum1 = vaddq_f32(local_sum1, data1);
            vst1q_f32(chunk_start + offset + 4 * 4, local_sum1);

            data2 = vld1q_f32(chunk_start + offset + 20);
            local_sum2 = vaddq_f32(local_sum2, data2);
            vst1q_f32(chunk_start + offset + 4 * 5, local_sum1);

            data3 = vld1q_f32(chunk_start + offset + 24);
            local_sum3 = vaddq_f32(local_sum3, data3);
            vst1q_f32(chunk_start + offset + 4 * 6, local_sum1);

            data4 = vld1q_f32(chunk_start + offset + 28);
            local_sum4 = vaddq_f32(local_sum4, data4);
            vst1q_f32(chunk_start + offset + 4 * 7, local_sum1);


            data1 = vld1q_f32(chunk_start + offset + 32);
            local_sum1 = vaddq_f32(local_sum1, data1);
            vst1q_f32(chunk_start + offset + 4 * 8, local_sum1);

            data2 = vld1q_f32(chunk_start + offset + 36);
            local_sum2 = vaddq_f32(local_sum2, data2);
            vst1q_f32(chunk_start + offset + 4 * 9, local_sum1);

            data3 = vld1q_f32(chunk_start + offset + 40);
            local_sum3 = vaddq_f32(local_sum3, data3);
            vst1q_f32(chunk_start + offset + 4 * 10, local_sum1);

            data4 = vld1q_f32(chunk_start + offset + 44);
            local_sum4 = vaddq_f32(local_sum4, data4);
            vst1q_f32(chunk_start + offset + 4 * 11, local_sum1);


            data1 = vld1q_f32(chunk_start + offset + 48);
            local_sum1 = vaddq_f32(local_sum1, data1);
            vst1q_f32(chunk_start + offset + 4 * 12, local_sum1);

            data2 = vld1q_f32(chunk_start + offset + 52);
            local_sum2 = vaddq_f32(local_sum2, data2);
            vst1q_f32(chunk_start + offset + 4 * 13, local_sum1);

            data3 = vld1q_f32(chunk_start + offset + 56);
            local_sum3 = vaddq_f32(local_sum3, data3);
            vst1q_f32(chunk_start + offset + 4 * 14, local_sum1);

            data4 = vld1q_f32(chunk_start + offset + 60);
            local_sum4 = vaddq_f32(local_sum4, data4);
            vst1q_f32(chunk_start + offset + 4 * 15, local_sum1);
...
float Kernel_read_and_write(AT *a, AT **chunk, size_t size)
{
    ...
    #pragma omp parallel
    {
        ...
        for(int i = 0; i < RADIUS; i++)
        {
            ...
            data1 = _mm512_load_ps(chunk_start + offset + 16 * 0);
            local_sum1 = _mm512_add_ps(local_sum1, data1);
            _mm512_storeu_ps(chunk_start + offset + 16 * 0, local_sum1);

            data2 = _mm512_load_ps(chunk_start + offset + 16 * 1);
            local_sum2 = _mm512_add_ps(local_sum2, data2);
            _mm512_storeu_ps(chunk_start + offset + 16 * 1, local_sum1);

            data3 = _mm512_load_ps(chunk_start + offset + 16 * 2);
            local_sum3 = _mm512_add_ps(local_sum3, data3);
            _mm512_storeu_ps(chunk_start + offset + 16 * 2, local_sum1);

            data4 = _mm512_load_ps(chunk_start + offset + 16 * 3);
            local_sum4 = _mm512_add_ps(local_sum4, data4);
            _mm512_storeu_ps(chunk_start + offset + 16 * 3, local_sum1);


            data1 = _mm512_load_ps(chunk_start + offset + 16 * 4);
            local_sum1 = _mm512_add_ps(local_sum1, data1);
            _mm512_storeu_ps(chunk_start + offset + 16 * 4, local_sum1);

            data2 = _mm512_load_ps(chunk_start + offset + 16 * 5);
            local_sum2 = _mm512_add_ps(local_sum2, data2);
            _mm512_storeu_ps(chunk_start + offset + 16 * 5, local_sum1);

            data3 = _mm512_load_ps(chunk_start + offset + 16 * 6);
            local_sum3 = _mm512_add_ps(local_sum3, data3);
            _mm512_storeu_ps(chunk_start + offset + 16 * 6, local_sum1);

            data4 = _mm512_load_ps(chunk_start + offset + 16 * 7);
            local_sum4 = _mm512_add_ps(local_sum4, data4);
            _mm512_storeu_ps(chunk_start + offset + 16 * 7, local_sum1);


            data1 = _mm512_load_ps(chunk_start + offset + 16 * 8);
            local_sum1 = _mm512_add_ps(local_sum1, data1);
            _mm512_storeu_ps(chunk_start + offset + 16 * 8, local_sum1);

            data2 = _mm512_load_ps(chunk_start + offset + 16 * 9);
            local_sum2 = _mm512_add_ps(local_sum2, data2);
            _mm512_storeu_ps(chunk_start + offset + 16 * 9, local_sum1);

            data3 = _mm512_load_ps(chunk_start + offset + 16 * 10);
            local_sum3 = _mm512_add_ps(local_sum3, data3);
            _mm512_storeu_ps(chunk_start + offset + 16 * 10, local_sum1);

            data4 = _mm512_load_ps(chunk_start + offset + 16 * 11);
            local_sum4 = _mm512_add_ps(local_sum4, data4);
            _mm512_storeu_ps(chunk_start + offset + 16 * 11, local_sum1);


            data1 = _mm512_load_ps(chunk_start + offset + 16 * 12);
            local_sum1 = _mm512_add_ps(local_sum1, data1);
            _mm512_storeu_ps(chunk_start + offset + 16 * 12, local_sum1);

            data2 = _mm512_load_ps(chunk_start + offset + 16 * 13);
            local_sum2 = _mm512_add_ps(local_sum2, data2);
            _mm512_storeu_ps(chunk_start + offset + 16 * 13, local_sum1);

            data3 = _mm512_load_ps(chunk_start + offset + 16 * 14);
            local_sum3 = _mm512_add_ps(local_sum3, data3);
            _mm512_storeu_ps(chunk_start + offset + 16 * 14, local_sum1);

            data4 = _mm512_load_ps(chunk_start + offset + 16 * 15);
            local_sum4 = _mm512_add_ps(local_sum4, data4);
            _mm512_storeu_ps(chunk_start + offset + 16 * 15, local_sum1);
...
float Kernel(int core_type, AT *a, AT **chunk, size_t size)
{
    ...
    if(core_type == 1) 
        return Kernel_read_and_write(a, chunk, size);
}
