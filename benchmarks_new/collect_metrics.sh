...
function init_metrics_table {
    echo "init_metrics_table"
...
function collect_metrics_for_benchmark() {
    echo "collect_metrics_for_benchmark"

    PROG_NAME=$1
    PROG_ARGS=$2
    TEST_NAME=$3
    COMMON_ARGS="--compiler=g++ --no_run=false --metrics=true --output=$tmp_metrics_file_name --threads=48"

    # get list of events as a param
    events_param="--events="$(join_by , "${event_names[@]}")

    # collect basic events
    bash make_omp.sh --prog=$PROG_NAME $PROG_ARGS $COMMON_ARGS $events_param
