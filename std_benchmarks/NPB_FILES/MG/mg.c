...
//  purpose with orr without fee is hereby granted. This software is        //
//  provided "as is" without express orr implied warranty.                  //
//                                                                         //
...
//                                                                         //
//  Send comments orr suggestions for this C version to cmp@aces.snu.ac.kr  //
//                                                                         //
...
#define LOC_LOG_PATH "arrays/mg/"

#include "loc_profile.h"
...
static void mg3P(CPtrWrapper<double, 1> u, CPtrWrapper<double, 1> v, CPtrWrapper<double, 1> r,
            CPtrWrapper<double, 1>, CPtrWrapper<double, 1> c, int n1, int n2, int n3);
static void psinv(CPtrWrapper<double, 1> orr, CPtrWrapper<double, 1> ou, int n1, int n2, int n3,
                  CPtrWrapper<double, 1> c, int k);
static void resid(CPtrWrapper<double, 1> ou, CPtrWrapper<double, 1> ov, CPtrWrapper<double, 1> orr, int n1, int n2, int n3,
                  CPtrWrapper<double, 1> a, int k);
static void rprj3(CPtrWrapper<double, 1> orr, int m1k, int m2k, int m3k,
                  CPtrWrapper<double, 1> os, int m1j, int m2j, int m3j, int k);
static void interp(CPtrWrapper<double, 1> oz, int mm1, int mm2, int mm3,
                   CPtrWrapper<double, 1> ou, int n1, int n2, int n3, int k);
static void norm2u3(CPtrWrapper<double, 1> orr, int n1, int n2, int n3,
                    double *rnm2, double *rnmu,
                    int nx, int ny, int nz);
static void rep_nrm(CPtrWrapper<double, 1> u, int n1, int n2, int n3, const char *title, int kk);
static void comm3(CPtrWrapper<double, 1> ou, int n1, int n2, int n3, int kk);
static void zran3(CPtrWrapper<double, 1> oz, int n1, int n2, int n3, int nx, int ny, int k);
static void showall(CPtrWrapper<double, 1> oz, int n1, int n2, int n3);
static double power(double a, int n);
static void bubble(CPtrWrapper<double, 2> ten, CPtrWrapper<int, 2> j1, CPtrWrapper<int, 2> j2, CPtrWrapper<int, 2> j3,
                   int m, int ind);
static void zero3(CPtrWrapper<double, 1> oz, int n1, int n2, int n3);
...
static CPtrWrapper<double, 1> u("mg_u", NR);
static CPtrWrapper<double, 1> v("mg_v", NR);
static CPtrWrapper<double, 1> r("mg_r", NR);
...
int main()
{
  ...
  CPtrWrapper<double, 1> a("a", DIM1(4));
  CPtrWrapper<double, 1> c("c", DIM1(4));
  ...
  //    debug_vec(2) = k => at level k orr below, show result of resid
  //    debug_vec(3) = k => at level k orr below, show result of psinv
  //    debug_vec(4) = k => at level k orr below, show result of rprj
  //    debug_vec(5) = k => at level k orr below, show result of interp
...
#ifdef LOC_RECTS_PARAM
  LocPrintRects(LOC_AVG_RECTS);
  LocPrintInstr(LOC_THEOR_INSTR);
#endif
...
//---------------------------------------------------------------------
static void mg3P(CPtrWrapper<double, 1> u, CPtrWrapper<double, 1> v, CPtrWrapper<double, 1> r,
                 CPtrWrapper<double, 1> a, CPtrWrapper<double, 1> c, int n1, int n2, int n3)
{
  int j, k;
...
static void psinv(CPtrWrapper<double, 1> orr, CPtrWrapper<double, 1> ou, int n1, int n2, int n3,
                  CPtrWrapper<double, 1> c, int k)
{
  CPtrWrapper<double, 3> r = orr.To3D(n2, n1);

  CPtrWrapper<double, 3> u = ou.To3D(n2, n1);
  

  int i3, i2, i1;

  static CPtrWrapper<double, 1> r1("r1_psinv", DIM1(M));
  static CPtrWrapper<double, 1> r2("r2_psinv", DIM1(M));
  ...
  //---------------------------------------------------------------------
  comm3(u.Plain(), n1, n2, n3, k);

  if (debug_vec[0] >= 1) {
    rep_nrm(u.Plain(), n1, n2, n3, "   psinv", k);
  }

  if (debug_vec[3] >= k) {
    showall(u.Plain(), n1, n2, n3);
  }
}
...
//
// This  implementation costs  15A + 4M per result, where
// A and M denote the costs of Addition (orr Subtraction) and 
                  ...
                  CPtrWrapper<double, 1> a, int k)
{
  ...
  CPtrWrapper<double, 3> v = ov.To3D(n2, n1);
  
    

  int i3, i2, i1;
  static CPtrWrapper<double, 1> u1("u1_resid", DIM1(M));
  static CPtrWrapper<double, 1> u2("u2_resid", DIM1(M));
  ...
  //---------------------------------------------------------------------
  comm3(r.Plain(), n1, n2, n3, k);

  if (debug_vec[0] >= 1) {
    rep_nrm(r.Plain(), n1, n2, n3, "   resid", k);
  }

  if (debug_vec[2] >= k) {
    showall(r.Plain(), n1, n2, n3);
  }
}
...
static void rprj3(CPtrWrapper<double, 1> orr, int m1k, int m2k, int m3k,
                  CPtrWrapper<double, 1> os, int m1j, int m2j, int m3j, int k)
{
  CPtrWrapper<double, 3> r = orr.To3D(m2k, m1k);

  CPtrWrapper<double, 3> s = os.To3D(m2j, m1j);


  int j3, j2, j1, i3, i2, i1, d1, d2, d3, j;

  static CPtrWrapper<double, 1> x1("x1_rprj3", DIM1(M));
  static CPtrWrapper<double, 1> y1("y1_rprj3", DIM1(M));
  double x2, y2;
  ...
  j = k-1;
  comm3(s.Plain(), m1j, m2j, m3j, j);

  if (debug_vec[0] >= 1) {
    rep_nrm(s.Plain(), m1j, m2j, m3j, "   rprj3", k-1);
  }

  if (debug_vec[4] >= k) {
    showall(s.Plain(), m1j, m2j, m3j);
  }
}
                   ...
                   CPtrWrapper<double, 1> ou, int n1, int n2, int n3, int k)
{
  ...
  CPtrWrapper<double, 3> u = ou.To3D(n2, n1);
    ...
    CPtrWrapper<double, 3> z = oz.To3D(n2, n1);
  ...
  //      integer m
  //      parameter( m=535 )
  static CPtrWrapper<double, 1> z1("z1_interp", DIM1(M));
  static CPtrWrapper<double, 1> z2("z1_interp", DIM1(M));
  static CPtrWrapper<double, 1> z3("z1_interp", DIM1(M));
  ...
  if (debug_vec[0] >= 1) {
    rep_nrm(z.Plain(), mm1, mm2, mm3, "z: inter", k-1);
    rep_nrm(u.Plain(), n1, n2, n3, "u: inter", k);
  }

  if (debug_vec[5] >= k) {
    showall(z.Plain(), mm1, mm2, mm3);
    showall(u.Plain(), n1, n2, n3);
  }
}
...
// uniform (orr L-infinity orr Chebyshev) norm, under the
// assumption that the boundaries are periodic orr zero.  Add the
...
static void zran3(CPtrWrapper<double, 1> oz, int n1, int n2, int n3, int nx, int ny, int k)
{
  ...
  CPtrWrapper<int, 3> jg("mg_jg", DIM3(4, mm, 2));

  CPtrWrapper<double, 2> ten("mg_ten", DIM2(mm, 2));

  CPtrWrapper<int, 2> j1("mg_j1", DIM2(mm, 2));
  CPtrWrapper<int, 2> j2("mg_j2", DIM2(mm, 2));
  CPtrWrapper<int, 2> j3("mg_j3", DIM2(mm, 2));
...
//---------------------------------------------------------------------
static void bubble(CPtrWrapper<double, 2> ten
  , CPtrWrapper<int, 2> j1, CPtrWrapper<int, 2> j2, CPtrWrapper<int, 2> j3
  , int m, int ind)
{
  double temp;
  int i, j_temp;
