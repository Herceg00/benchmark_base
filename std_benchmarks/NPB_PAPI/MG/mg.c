...
//  purpose with or_ without fee is hereby granted. This software is        //
//  provided "as is" without express or_ implied warranty.                  //
//                                                                         //
...
//                                                                         //
//  Send comments or_ suggestions for this C version to cmp@aces.snu.ac.kr  //
//                                                                         //
...
static void psinv(void *or_, void *ou, int n1, int n2, int n3,
                  double c[4], int k);
static void resid(void *ou, void *ov, void *or_, int n1, int n2, int n3,
                  double a[4], int k);
static void rprj3(void *or_, int m1k, int m2k, int m3k,
...
static void norm2u3(void *or_, int n1, int n2, int n3,
...
LOC_PAPI_INIT
  ...
  //    debug_vec(2) = k => at level k or_ below, show result of resid
  //    debug_vec(3) = k => at level k or_ below, show result of psinv
  //    debug_vec(4) = k => at level k or_ below, show result of rprj
  //    debug_vec(5) = k => at level k or_ below, show result of interp
  ...
  printf(" Size: %4dx%4dx%4d  (class_ %c)\n", nx[lt], ny[lt], nz[lt], Class);
...
static void psinv(void *or_, void *ou, int n1, int n2, int n3,
                  double c[4], int k)
{
  double (*r)[n2][n1] = (double (*)[n2][n1])or_;
...
//
// This  implementation costs  15A + 4M per result, where
// A and M denote the costs of Addition (or_ Subtraction) and 
...
static void rprj3(void *or_, int m1k, int m2k, int m3k,
                  void *os, int m1j, int m2j, int m3j, int k)
{
  double (*r)[m2k][m1k] = (double (*)[m2k][m1k])or_;
...
// uniform (or_ L-infinity or_ Chebyshev) norm, under the
// assumption that the boundaries are periodic or_ zero.  Add the
