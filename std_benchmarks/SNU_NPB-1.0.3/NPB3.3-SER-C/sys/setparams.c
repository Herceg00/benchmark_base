...
void write_ua_i4fo(FILE *fp, char class);
...
void write_sp_info(FILE *fp, char class) 
{
  ...
  char *dt;
  if      (class == 'S') { problem_size = 12;  dt = "0.015";   niter = 1; }
...
void write_bt_info(FILE *fp, char class) 
{
  int problem_size, niter;
  char *dt;
  if      (class == 'S') { problem_size = 4;  dt = "0.010";   niter = 1; }
...
void write_mg_info(FILE *fp, char class) 
{
  ...
  if      (class == 'S') { problem_size = 16; nit = 1; }
/*  else if (class == 'W') { problem_size = 64; nit = 40; }*/
  else if (class == 'W') { problem_size = 64; nit = 1; }
...
void write_cg_info(FILE *fp, char class) 
{
  ...
  if( class == 'S' )
  { na=100; nonzer=5; niter=1; shift=shiftS; }
  else if( class == 'W' )
...
void write_ua_info(FILE *fp, char class) 
{
  ...
  fre = 5;
  if( class == 'S' )
  { lelt=250;lmor=11600;       refine_max=4;  niter=1;  nmxh=10; alpha="0.040e0"; }
  else if( class == 'W' )
...
void write_ft_info(FILE *fp, char class) 
{
  ...
  if      (class == 'S') { nx = 64; ny = 64; nz = 64; niter = 1;}
