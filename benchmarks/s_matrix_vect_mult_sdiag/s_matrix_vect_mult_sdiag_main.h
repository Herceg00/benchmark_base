...
#include "s_matrix_vect_mult_sdiag.h"

double CallKernel()
{
	SparseSDiagMatrixSpec spec = GenerateSpec(N, SPARSITY);

	SparseSDiagMatrix<data_arr, index_arr> matrix;

	matrix.a = new base_type[spec.a];
	matrix.b = new base_type[spec.b];
	matrix.x = new base_type[spec.x];
	matrix.ibeg = new index_type[spec.ibeg];
	matrix.ip = new index_type[spec.ip];
	matrix.ic = new index_type[spec.ic];
