...
#include "tridiagonal_other.h"

double CallKernel()
{
	vector_type u_diag("u_diag", DIM1(N + 1));
	vector_type diag("diag", DIM1(N + 1));
	vector_type l_diag("l_diag", DIM1(N + 1));
	vector_type result("result", DIM1(N + 1));

	timeval start, end;

	Init<base_type, vector_type>(u_diag, diag, l_diag, result, N);
...
gettimeofday(&start, NULL);
	...
	Kernel<base_type, vector_type>(u_diag, diag, l_diag, result, N);
